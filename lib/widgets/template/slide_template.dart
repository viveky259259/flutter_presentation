...
class SlideTemplate extends StatelessWidget implements IPageController {
  ...
  SlideTemplate({
    ...
    required this.next,
    required this.prev,
    this.borderColor = const Color(0xfff6e8b1),
  }) : super(key: key);

  // static Color borderColor = const Color(0xfff6e8b1);
    ...
    return RawKeyboardListener(
      ...
      onKey: (value) {
        ...
        if (lastActionTime == null ||
            lastActionTime!.difference(DateTime.now()).inSeconds > 2) {
          lastActionTime = DateTime.now();
          var data = value.data;
          if (data is RawKeyEventDataWeb) {
            if (data.code == 'ArrowRight') {
              clicked(Directions.right);
            } else if (data.code == 'ArrowLeft') {
              clicked(Directions.left);
            }
          }
        }
      },
